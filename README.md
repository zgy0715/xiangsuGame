...
单测覆盖:全部关卡唯一解与答案一致、提示逐步推导收敛到答案、DTO↔JSON 契约、额度每日规则、对战帧协议(含重赛/NTP 校时)、会话状态机、蓝牙掩码校验与结算排序(40+ 用例);服务端冒烟覆盖昵称持久化、房间发车/结算/重赛/断线重进/房主宽限回收。`pip install -r requirements.txt` 后 `cd server && python smoke_test.py` 即可验证。
